...
struct sockaddr_in server_addr_udp[5];
...
static bool
stream_recv_packet_udp(struct stream *stream, AVPacket *packet) {
    uint32_t frame_size = 0;
    uint8_t frame_data[1048576] = {0}; // 1MB is (hopefully) enough for one frame // use unsigned char to prevent "ffffffe0" and so on (we want "e0")
    uint16_t fragment_trace = 0x0000; // used ONLY for detecting position
	...
	socklen_t addr_sz = sizeof(server_addr_udp[stream->udp_index]);
	int len = recvfrom(stream->socket, buffer, 2000, 0,
				(struct sockaddr*)&server_addr_udp[stream->udp_index], &addr_sz);
        if (len == 0) {
            printf("connection terminated by server.\n");
            close(stream->socket);
            exit(0);
...
run_stream_udp(void *data) {
    ...
    LOGI("run_stream_udp for index %d", stream->udp_index);
    for (;;) {

	
        AVPacket packet;
        bool ok = stream_recv_packet_udp(stream, &packet);
        if (!ok) {
            // end of stream
            break;
        }
...
stream_init_udp(struct stream *stream, socket_t msocket,
    ...
    memset(&server_addr_udp[index], 0, sizeof(server_addr_udp[index]));
    server_addr_udp[index].sin_family = AF_INET;
    server_addr_udp[index].sin_addr.s_addr = htonl(INADDR_ANY);
    server_addr_udp[index].sin_port = htons(sessionPort);

    LOGI("video_socket connect() try\n");
    while (bind(msocket, (struct sockaddr *)&server_addr_udp[index], sizeof(server_addr_udp[index])) < 0);
    LOGI("video_socket connect() success\n");
    stream->socket = msocket;
    stream->decoder = decoder,
    stream->recorder = recorder;
    stream->has_pending = false;
    stream->udp_port = sessionPort;
    stream->udp_index = index;
    LOGI("%d udp bind good!!", index);
}
...
stream_start_udp(struct stream *stream) {
    ...
    stream->thread = SDL_CreateThread(run_stream_udp, "stream", stream);
    if (!stream->thread) {
        LOGC("Could not start stream thread");
        return false;
    }
